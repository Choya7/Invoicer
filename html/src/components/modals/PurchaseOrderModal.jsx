...
import { updatePurchaseOrder } from '../../core/database';
...
export default function PurchaseOrderModal({ isOpen, onClose, po, onSaved }) {
  ...
  useEffect(() => {
    if (po && isOpen) {
      setIssueDate(new Date(po.issue_date));
      setClientName(po.client_name);
      setSupplier(po.data.supplier);
      setRows(po.data.items.map(it => ({...it})));
      setFooter(po.data.footer || { prev_balance: 0, total_sum: 0, deposit: 0, total_balance: 0, receiver: '' });
      setGrandTotal(po.data.grand_total);
    }
  }, [po, isOpen]);
  ...
  const handleUpdate = async () => {
    ...
    if (validRows.length === 0) {
      MySwal.fire('수정 실패', '발주서에 최소 1개 이상의 물품 내역을 작성해주세요.', 'error');
      return;
    }
    ...
    await updatePurchaseOrder(po.id, issueDate.toISOString().split('T')[0], clientName, data);
    MySwal.fire('수정 완료', '발주서 정보가 업데이트되었습니다.', 'success');
    onSaved();
    onClose();
  };
  ...
  const insertRow = () => {
    ...
    setRows([...rows, { date: dStr, code: '', name: '', name: '', spec: '', qty: '', completed_qty: 0, price: '', supply: 0, tax: 0, note: '' }]);
  };
      ...
      <div className="modal-content" style={{ maxWidth: '1200px', width: '95%', maxHeight: '95vh', overflow: 'auto' }}>
        <div className="flex justify-between items-center mb-4 border-b pb-2">
          <h2 className="text-xl font-bold text-emerald-600 flex items-center gap-2">
            발주서 상세 및 수정 <span className="text-xs font-normal text-muted">ID: {po?.id}</span>
          </h2>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full"><X size={20}/></button>
        </div>

        <div className="flex-responsive justify-between items-center mb-6 bg-emerald-50 p-3 rounded-lg border border-emerald-200 gap-4">
          <div className="flex flex-wrap gap-2 justify-center md:justify-start">
            <button className="btn btn-primary" style={{ backgroundColor: '#10b981', borderColor: '#10b981' }} onClick={handleUpdate}><Save size={16}/> 수정 내용 저장</button>
            <button className="btn" onClick={handlePrintExcel}><Printer size={16}/> 이 건만 별도 출력 (Excel)</button>
          </div>
          <div className="flex gap-4 items-center justify-center">
            <label className="flex items-center gap-2 text-sm font-semibold">
              <input type="radio" name="taxTypeEditPO" checked={taxCalcType === 'separate'} onChange={() => setTaxCalcType('separate')} />
              VAT 별도
            </label>
            <label className="flex items-center gap-2 text-sm font-semibold">
              <input type="radio" name="taxTypeEditPO" checked={taxCalcType === 'include'} onChange={() => setTaxCalcType('include')} />
              VAT 포함
            </label>
          </div>
        </div>

        <div className="card p-4 md:p-6 shadow-sm border-t-4 border-emerald-500">
          <div className="flex-responsive gap-6 mb-6">
            <div className="flex-1 border-2 border-slate-200 rounded-lg p-3 md:p-4">
              <div className="text-center mb-4">
                <h3 className="text-xl font-bold underline underline-offset-8 decoration-emerald-500">발 주 서</h3>
              </div>
              <div className="grid md-grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-1 flex-1">
                    ...
                    <button className="btn btn-primary p-1" style={{ backgroundColor: '#10b981', borderColor: '#10b981' }} onClick={() => setClientModalOpen(true)}><Search size={16}/></button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 flex gap-4 border-2 border-slate-200 rounded-lg p-3 md:p-4">
               <div className="bg-emerald-500 text-white font-bold p-3 rounded flex items-center justify-center text-center leading-loose mobile-hide" style={{ writingMode: 'vertical-rl', letterSpacing: '0.3em' }}>공급자</div>
          ...
          <div className="mb-4 bg-emerald-50 border border-emerald-200 p-3 rounded-lg flex justify-between items-center">
             <span className="font-bold text-emerald-900">총 발주금액</span>
            ...
            <table className="table table-responsive">
              <thead>
                <tr className="bg-slate-100">
                  ...
                  <th style={{ width: '100px' }}>작업수량</th>
                  <th style={{ width: '100px' }}>잔량</th>
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i}>
                    ...
                    <td data-label="작업수량" className="p-1"><input className="form-input text-xs text-right p-1 h-8 bg-emerald-50 font-bold" type="number" value={row.completed_qty} onChange={e => handleRowChange(i, 'completed_qty', e.target.value)} /></td>
                    <td data-label="잔량" className="p-1"><input className="form-input text-xs text-right p-1 h-8 bg-gray-50" value={(parseFloat(row.qty || 0) - parseFloat(row.completed_qty || 0)).toLocaleString()} readOnly /></td>
